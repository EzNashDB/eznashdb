...
def test_shows_app_name(GET_request):
    ...
    soup = BeautifulSoup(str(response.render().content),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("has_kaddish_with_men", True, "fa-check"),
        ("has_kaddish_with_men", False, "fa-times"),
        ("enum_has_kaddish_alone", KaddishAlone.YES, str(KaddishAlone.YES)),
        ("enum_has_kaddish_alone", KaddishAlone.NO, str(KaddishAlone.NO)),
        ("enum_has_kaddish_alone", KaddishAlone.MAN_ALWAYS_SAYS_KADDISH,
         str(KaddishAlone.MAN_ALWAYS_SAYS_KADDISH)),
    ],
)
